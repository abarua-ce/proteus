...
if nd == 1:
    ...
    vertices = np.loadtxt('filtered_vertices.txt', delimiter=',').tolist()
    vertexFlags = []
    marker = np.loadtxt('updated_vertex_flags.txt', dtype=int, skiprows=1).tolist()
    for value in marker:
        formatted_value = f"marker_{value}"
        vertexFlags.append(boundaryTags[formatted_value])
    segments=np.loadtxt('filtered_segments.txt', dtype=int).tolist()
    segmentFlags = []
    ...
    regions = [
        [0.26422764227642276, 0.52],#1
        [0.26422764227642276, 0.42], #2
        [0.18292682926829268, 0.3461788617886179],#3
        [0.426829268292683, 0.33552845528455284], #4
        [0.6300813008130082, 0.353], #5
        [0.1016260162601626, 0.32552845528455284], #6
        [0.38617886178861793, 0.29],#7
        [0.8333333333333335, 0.335],#8
        [0.14227642276422764, 0.305],#9
        [0.426829268292683, 0.265], #10
        [0.6300813008130082, 0.324],#11
        [0.10975609756097562, 0.27],#12
        [0.426829268292683, 0.23],#13
        [0.5975609756097562, 0.28],#14
        [0.9, 0.255], #15
        [0.01, 0.20], #16
        [0.20, 0.20], #17
        [0.08, 0.232], #18
        [0.1, 0.22], #19
        [0.2, 0.23], #20
        [0.01, 0.12195121951219512], #21
        [0.18292682926829268, 0.1626016260162602], #22
        [0.03, 0.060975609756097573], #23
        [0.2682926829268293, 0.0813008130081301], #24
        [0.6829268292682927, 0.02], #25
        [0.15, 0.15], #26
        [0.3292682926829268, 0.34552845528455284], #27
        [0.524390243902439, 0.3252032520325203],#28
    ]
    ...
    triangleOptions="pAq30Dena%8.8f"  % 0.005
