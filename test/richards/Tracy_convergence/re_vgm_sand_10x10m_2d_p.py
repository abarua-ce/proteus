...
psi_r   = -3.0      # reference head used in Tracy (choose your value, e.g., -2.0)
alpha_g = 0.5      # Gardner alpha [1/m] used in the analytical BC formula
beta_bc = 2.0      # scaling (>1 => "higher" top head); optional convenience
hat_h0  = beta_bc * (1.0 - numpy.exp(alpha_g*psi_r))  # amplitude in transformed variable

a_span  = L[0]     # domain width 'a' (here = 10.0)


def getDBC_Tracy(x, flag):
