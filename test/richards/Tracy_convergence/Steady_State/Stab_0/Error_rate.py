...
# ----------------------------------------------------------------------
# STEADY Tracy evaluator: uses ONLY the steady part (no transient term)
# (keeps the same masking style you already have)
# ----------------------------------------------------------------------
def _tracy_eval(nodes, params):
    """
    Vectorized evaluation of Tracy STEADY solution at a set of points.
    ...
    # --- top boundary: direct BC formula (time-independent) ---
    if np.any(top_mask):
        x_top = x[top_mask]
    ...
    # --- interior points: steady transformed part only ---
...
def tracy_steady(nodes, * ,
                 a, L, alpha, psi_r,
                 theta_s, theta_r, Ks_day,
                 n_terms=120):
    """
    Steady Tracy solution psi(x,z) for each node.
    NOTE: t_days is not needed; we pass 0.0 into precompute for compatibility.
    """
...
# ----------------------------------------------------------------------
# L2 / Linf error using quadpy with precomputed Tracy params (STEADY)
# ----------------------------------------------------------------------
def tracy_steady_L2_Linf_error_quadpy_psi(u_num,
                                          nodes,
                                          elements,
                                          *,
                                          a, L, alpha, psi_r,
                                          ...
                                          degree=5):
    """
    Steady Tracy error in psi only: returns (L2_psi, Linf_psi)
    """
...
# ONLY this index (as requested)
time_indices = [201]

OUT_TXT = "Tracy_steady_L2_Linf_convergence_steady_state.txt"
...
with open(OUT_TXT, "w") as fout:
    ...
    for t_idx in time_indices:
        ...
        fout.write(f"\n===== Errors vs STEADY Tracy at t = {t_days:.6e} days (index={t_idx}) =====\n")
        results = []
        for ref_name, nnx in ref_info:
            ...
            L2_psi, Linf_psi = tracy_steady_L2_Linf_error_quadpy_psi(
                psi_num,
                nodes,
                elements,
                a=10.0,
