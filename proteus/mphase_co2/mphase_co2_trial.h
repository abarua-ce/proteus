...
// First make a loop in the calculateResidual function for two phase air and water 
  void calculateResidual(arguments_dict &args)
  {
    ...
    xt::pyarray<int> &csrRowIndeces_CellLoops_water       = args.array<int>("csrRowIndeces_CellLoops_water");
    xt::pyarray<int> &csrColumnOffsets_CellLoops_water    = args.array<int>("csrColumnOffsets_CellLoops_water");
    xt::pyarray<int> &csrColumnOffsets_eb_CellLoops_water = args.array<int>("csrColumnOffsets_eb_CellLoops_water");
    ...
    xt::pyarray<int> &csrRowIndeces_CellLoops_air       = args.array<int>("csrRowIndeces_CellLoops_air");
    xt::pyarray<int> &csrColumnOffsets_CellLoops_air    = args.array<int>("csrColumnOffsets_CellLoops_air");
    xt::pyarray<int> &csrColumnOffsets_eb_CellLoops_air = args.array<int>("csrColumnOffsets_eb_CellLoops_air");
 ...
 //   xt::pyarray<double> &q_dV_air                                   = args.array<double>("q_dV_air");
    ...
    for (int eN = 0; eN < nElements_global; eN++) {
      ...
      for (int k = 0; k < nQuadraturePoints_element; k++) {
        ...
        ck.calculateMapping_element(eN, 
                                    ...
                                    jac, jacDet, jacInv, 
                                    x, y, z);
        ck.calculateMappingVelocity_element(eN, k, 
        ...
        //get the solution for both phas`e
  ...
  //      ck.gradFromDOF(u_dof.data(), &u_l2g.data()[eN_nDOF_trial_element], u_grad_trial, grad_u);
       ...
       //                      thetaSR.data()[elementMaterialTypes.data()[eN]], &KWs.data()[elementMaterialTypes.data()[eN] * nnz], u, m, dm, f, df, a, da, as, Kr, dKr);
        ...
        //
        ck.bdf(alphaBDF, 
               q_m_betaBDF_water.data()[eN_k], 
               m_water, 
               dm_water, 
               m_t_water, 
               dm_t_water);
        ck.bdf(alphaBDF, 
               q_m_betaBDF_air.data()[eN_k], 
               m_air, 
               dm_air, 
               m_t_air, 
               dm_t_air);
        ...
        pdeResidual_u_water = ck.Mass_strong(m_t_water) + 
                              ck.Advection_strong(df_water, grad_u_water);      
        pdeResidual_u_air = ck.Mass_strong(m_t_air) + 
                            ck.Advection_strong(df_air, grad_u_air);
        ...
        for (int i = 0; i < nDOF_test_element; i++) {
          ...
          elementResidual_u_water[i] += ck.Mass_weak(m_t_water, u_test_dV[i]) +
                                         ck.Advection_weak(f_water, &u_grad_test_dV[i_nSpace]) + 
                                         ck.Diffusion_weak(a_rowptr.data(), 
                                         a_colind.data(), 
                                         a_water, grad_u_water, &u_grad_test_dV[i_nSpace]) +
                                         VMS * ck.SubgridError(subgridError_u_water, Lstar_u_water[i]) + 
                                         VMS * ck.NumericalDiffusion(q_numDiff_u_last_water[eN_k], grad_u_water, &u_grad_test_dV[i_nSpace]);
          elementResidual_u_air[i] += ck.Mass_weak(m_t_air, u_test_dV[i]) + 
                                      ck.Advection_weak(f_air, &u_grad_test_dV[i_nSpace]) + 
                                      ck.Diffusion_weak(a_rowptr.data(), a_colind.data(), 
                                                        a_air, 
                                                        grad_u_air, 
                                                        &u_grad_test_dV[i_nSpace])  + 
                                      VMS * ck.SubgridError(subgridError_u_air, Lstar_u_air[i]) + 
                                      VMS * ck.NumericalDiffusion(q_numDiff_u_last_air[eN_k], grad_u_air, &u_grad_test_dV[i_nSpace]);
        ...
        //phase air
        q_m_air.data()[eN_k+1] = m_air;
        q_u_air.data()[eN_k+1] = u_air;
      }
      //
    ...
    for (int eN = 0; eN < nElements_global; eN++) {
      ...
      for (int k = 0; k < nQuadraturePoints_element; k++) {
        ...
        for (int j = 0; j < nDOF_trial_element; j++) {
          ...
          dpdeResidual_u_u_water[j] = ck.MassJacobian_strong(dm_t_water, u_trial_ref[k * nDOF_trial_element + j]) + 
                                      ck.AdvectionJacobian_strong(df_water, &u_grad_trial[j_nSpace]);
          dpdeResidual_u_u_air[j] = ck.MassJacobian_strong(dm_t_air, u_trial_ref[k * nDOF_trial_element + j]) +   
...
//    xt::pyarray<double> &delta_x_ij_air = args.array<double>("delta_x_ij_air");
    ...
    xt::pyarray<double> &mn_water        = args.array<double>("mn_water");
    xt::pyarray<double> &mn_air        = args.array<double>("mn_air");
    ...
    for (int i = 0; i < numDOFs; i++) {
      ...
      if (STABILIZATION_TYPE == STABILIZATION::EV_Stab) //EV stab
      {
        ...
        eta_water[i]                      = ENTROPY_TYPE == 1 ? ENTROPY(solni_water, uL, uR) : ENTROPY_LOG(solni_water, uL, uR);
        eta_air[i]                        = ENTROPY_TYPE == 1 ? ENTROPY(solni_air, uL, uR) : ENTROPY_LOG(solni_air, uL, uR);
    ...
    for (int eN = 0; eN < nElements_global; eN++) {
      ...
      for (int k = 0; k < nQuadraturePoints_element; k++) {
       ...
       // calculateCFL(elementDiameter.data()[eN] / degree_polynomial, velocity_loc, cfl.data()[eN_k]);
        ...
        if (STABILIZATION_TYPE == STABILIZATION::EV_Stab) // EV stab
        {
          ...
          DENTROPY_un_water = ENTROPY_TYPE == 1 ? DENTROPY(un_water, uL, uR) : DENTROPY_LOG(un_water, uL, uR);
          DENTROPY_un_air = ENTROPY_TYPE == 1 ? DENTROPY(un_air, uL , uR) : DENTROPY_LOG(un_air, uL, uR);        
        }
        //////////////
        // ith-LOOP //
        //////////////
        for (int i = 0; i < nDOF_test_element; i++) {
          ...
          if (STABILIZATION_TYPE == STABILIZATION::EV_Stab) // EV stab
          {
            ...
            DENTROPY_uni_water              = ENTROPY_TYPE == 1 ? DENTROPY(uni_water, uL, uR) : DENTROPY_LOG(uni_water, uL, uR);           
            DENTROPY_uni_air              = ENTROPY_TYPE == 1 ? DENTROPY(uni_air, uL, uR) : DENTROPY_LOG(uni_air, uL, uR);
      ...
      for (int i = 0; i < nDOF_test_element; i++) {
        ...
        for (int j = 0; j < nDOF_trial_element; j++) {
          int eN_i_j = eN_i * nDOF_trial_element + j;
        ///phase 0::water
          TransportMatrix_water[csrRowIndeces_CellLoops_water.data()[eN_i] + csrColumnOffsets_CellLoops_water.data()[eN_i_j]] += elementTransport_water[i][j];
          TransportMatrixConsistent_water[csrRowIndeces_CellLoops_water.data()[eN_i] + csrColumnOffsets_CellLoops_water.data()[eN_i_j]] += elementTransportConsistent_water[i][j];
          TransportMatrixn_water[csrRowIndeces_CellLoops_water.data()[eN_i] + csrColumnOffsets_CellLoops_water.data()[eN_i_j]] += elementTransportn_water[i][j];
          TransportMatrixConsistentn_water[csrRowIndeces_CellLoops_water.data()[eN_i] + csrColumnOffsets_CellLoops_water.data()[eN_i_j]] += elementTransportConsistentn_water[i][j];
        ///phase 1: air
          TransportMatrix_air[csrRowIndeces_CellLoops_air.data()[eN_i] + csrColumnOffsets_CellLoops_air.data()[eN_i_j]] += elementTransport_air[i][j];
          TransportMatrixConsistent_air[csrRowIndeces_CellLoops_air.data()[eN_i] + csrColumnOffsets_CellLoops_air.data()[eN_i_j]] += elementTransportConsistent_air[i][j];
          TransportMatrixn_air[csrRowIndeces_CellLoops_air.data()[eN_i] + csrColumnOffsets_CellLoops_air.data()[eN_i_j]] += elementTransportn_air[i][j];
          TransportMatrixConsistentn_air[csrRowIndeces_CellLoops_air.data()[eN_i] + csrColumnOffsets_CellLoops_air.data()[eN_i_j]] += elementTransportConsistentn_air[i][j];                
        } //j
      } //i
    } //elementsxw
...
//        bc_u_ext = isDOFBoundary_u.data()[ebNE_kb] * ebqe_bc_u_ext.data()[ebNE_kb] + (1 - isDOFBoundary_u.data()[ebNE_kb]) * u_ext;
    ...
    for (int i = 0; i < numDOFs; i++) {
      ...
      for (int offset = csrRowIndeces_DofLoops_water.data()[i]; offset < csrRowIndeces_DofLoops_water.data()[i + 1]; offset++) { // second loop in j (sparsity pattern)
        ...
        double gi_times_x = 0.;
        for (int I = 0; I < nSpace; I++) {
          gi_times_x += gi_water[I] * delta_x_ij.data()[offset * 3 + I];
        }
...
// =============== air ================= //
    ...
    for (int i = 0; i < numDOFs; i++) {
      ...
      for (int offset = csrRowIndeces_DofLoops_air.data()[i]; offset < csrRowIndeces_DofLoops_air.data()[i + 1]; offset++) { // second loop in j (sparsity pattern)
        ...
        double gi_times_x = 0.;
        for (int I = 0; I < nSpace; I++) {
          gi_times_x += gi_air[I] * delta_x_ij.data()[offset * 3 + I];
        }
        // compute the positive and negative part of gi*(xi-xj)
        SumPos += gi_times_x > 0 ? gi_times_x : 0;
        SumNeg += gi_times_x < 0 ? gi_times_x : 0;
