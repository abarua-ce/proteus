...
class RKEV(TimeIntegration.SSP):
    ...
    def choose_dt(self):
        ...
        self.dt = self.runCFL/ maxCFL # old_div(self.runCFL, maxCFL)
    ...
    def updateStage(self):
        ...
        if self.timeOrder == 3:
            if self.lstage == 1:
                ...
                for ci in range(self.nc):
                    self.u_dof_stage[ci][self.lstage][:] = self.transport.u[ci].dof
                    self.u_dof_stage[ci][self.lstage] *= 1./4. #old_div(1., 4.)
                ...
                for ci in range(self.nc):
                    self.u_dof_stage[ci][self.lstage][:] = self.transport.u[ci].dof
                    self.u_dof_stage[ci][self.lstage] *= 2.0/3.0 #old_div(2.0, 3.0)
            ...
            if self.lstage == 1:
                ...
                for ci in range(self.nc):
                    self.u_dof_stage[ci][self.lstage][:] = self.transport.u[ci].dof
                    self.u_dof_stage[ci][self.lstage][:] *= 1.0/2.0 #old_div(1., 2.)
...
#        self.q[('dV_u', 0)] = (old_div(1.0, self.mesh.nElements_global)) * np.ones((self.mesh.nElements_global, self.nQuadraturePoints_element), 'd')
        ...
        self.richards_sat = cRichards_sat_base(self.nSpace_global,
    ...
    def FCTStep(self):
        ...
        #pdb.set_trace()
        self.richards_sat.FCTStep(argsDict)
    ...
    def kth_FCT_step(self):
        ...
        self.richards_sat.kth_FCT_step(
            self.timeIntegration.dt,
    ...
    def getResidual(self,u,r):
        ...
        if (self.coefficients.STABILIZATION_TYPE == 0):  # SUPG
            self.calculateResidual = self.richards_sat.calculateResidual
            self.calculateJacobian = self.richards_sat.calculateJacobian
        else:
            self.calculateResidual = self.richards_sat.calculateResidual_entropy_viscosity
            self.calculateJacobian = self.richards_sat.calculateMassMatrix
    ...
    def invert(self,u,r):
        ...
        argsDict["sLow"] = self.sLow
        argsDict["sn"] = self.sn
        self.richards_sat.invert(argsDict)
            # self.timeIntegration.dt,
