...
# BOUNDARY CONDITIONS #
#######################
# def setboundarycondition(x):
#     # Boundary condition at the left (x=0) and bottom (y=0)
#     if x[0] == 0.0 :
#         return lambda x, t: 1.0
#     else:
#         return None
#     # # Boundary condition at the right (x=1) and top (y=1)
#     # elif x[0] == L[0] or x[1] == L[0]:
#     #     return lambda x, t: 0.0

def getDBC(x,flag):
    if ct.problem in [0,2]:
        if flag == domain.boundaryTags['left']:
            #print(f"at left Applying boundary condition at x = {x}")    
            return lambda x,t:1.0
        if flag == domain.boundaryTags['top']:
            #print(f"at top Applying boundary condition at x = {x}")
            return lambda x,t:0.0
        if flag == domain.boundaryTags['bottom']:
            #print(f"at bottom Applying boundary condition at x = {x}")
            return lambda x,t:1.0
        if flag == domain.boundaryTags['right']:
            #print(f"at right Applying boundary condition at x = {x}")
            return lambda x,t:0.0
        else:
            return None
